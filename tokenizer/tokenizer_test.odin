...
expect_tokens :: proc(t: ^testing.T, src: string, expected: []Token_Kind, loc := #caller_location) {
	tokens := tokenize_string(src)
	defer delete(tokens)
	
	if len(tokens) != len(expected) + 1 { // +1 для EOF
		testing.expect_assert_message(t, fmt.aprintf("Expected %d tokens (+ EOF), got %d", len(expected), len(tokens)-1), loc)
	...
	for kind, i in expected {
		if tokens[i].kind != kind {
			testing.expectf(
				...
				i, kind, tokens[i].kind, tokens[i].text,
				loc=loc,
			)
		}
	}
